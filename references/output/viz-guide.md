...
`{skill_dir}/scripts/build_viz.py` is local-only (it never calls the FactIQ
API). Resolve `{skill_dir}` as described in `SKILL.md`; do not use the
...
3. **Assemble** the self-contained file. Bespoke vizzes usually combine
   ...
   ```bash
   python3 "{skill_dir}/scripts/build_viz.py" assemble \
...
4. **Render and look.** Screenshot it, then actually read the image:
   ```bash
   python3 "{skill_dir}/scripts/build_viz.py" render /tmp/out.html --out /tmp/out.png
   ```
...
python3 "{skill_dir}/scripts/build_viz.py" save --tool run_sql --out /tmp/korea.json
```
...
- `--match STR` pins the exact call by a substring of its **input** — a schema
  ...
  python3 "{skill_dir}/scripts/build_viz.py" save --match "census" --out /tmp/census.json
  ```
