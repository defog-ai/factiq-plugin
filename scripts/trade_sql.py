...
def load_partners(source: str) -> tuple[dict[str, str], dict[str, str]]:
    ...
    try:
        ...
        return data["partners"][key], data["aliases"][key]
...
def resolve_partner(source: str, text: str) -> tuple[str, str]:
    ...
    fail(f"'{text}' is ambiguous in '{source}' ({len(hits)} matches): {listing}", code=2)
    raise AssertionError  # unreachable
...
def header(source: str, args: argparse.Namespace, partner_name: str, level: int, extra: str = "") -> str:
    cfg = SOURCES[source]
    lines = [
...
def sql_total(args: argparse.Namespace) -> str:
    ...
    if args.monthly:
        select, group = "dp.time::date AS month, SUM(dp.value) AS value", "\nGROUP BY 1\nORDER BY 1"
    else:
...
def sql_products(args: argparse.Namespace) -> str:
    ...
    if group_digits > level:
        fail(f"--group-by {group_digits} is finer than the HS-{level} lines being queried")
    lo, hi = date_bounds(args.start, args.end)
...
def sql_trend(args: argparse.Namespace) -> str:
    ...
    if len(code) in cfg["levels"] and args.metric == "value":
        # Full-length code at a stored level: the series ID is constructible.
        level = len(code)
        series_id = f"{cfg['prefix']}_{flow_code(args.flow)}_{level}d_{code}_{partner_code}"
        body = (
...
def main() -> None:
    ...
    def add_common(p: argparse.ArgumentParser) -> None:
        ...
        p.add_argument(
            ...
            "'world' = the all-partner total row (census/china only)",
        )
        ...
        p.add_argument("--level", type=int, help="HS digit level (default: the source's international level)")
    ...
    p_products.add_argument("--group-by", default="2", choices=["2", "4", "6"], help="HS digits to group by (default 2)")
    ...
    add_common(p_trend)
    p_trend.add_argument("--hs", required=True, help="HS code: full-length = exact series, 2/4/6-digit = summed group")
    p_trend.add_argument("--metric", default="value", choices=["value", "quantity"], help="default: value")
