...
- For media, narrow by ticker, person, channel, institution, dates, target,
  appearance type, and claim family, or read the next page with
  `offset=next_offset` while the result says `has_more`. Synthesize multiple
  bounded searches; never use SQL or assume a full-transcript path.
...
| `claims` | Structured, decision-relevant claims by executives, investors, analysts, central-bank officials, and other guests. Empty query browses recent claims |
...
All targets accept `company`, `person`, `channel` (podcast show name,
YouTube channel, or institution name), `appearance_type`, `claim_family`
where compatible, publication-date `date_from`/`date_to`, `limit`, and
...
`offset=next_offset`, or narrow by ticker, person, channel, target,
