...
- For media, narrow by ticker, person, show, institution, dates, target,
...
All targets accept `company`, `person`, `show`,
`appearance_type`, `claim_family` where compatible, publication-date
`date_from`/`date_to`, `limit`, and `offset`; finding targets also support
`detail`.
...
`offset=next_offset`, or narrow by ticker, person, show, target, appearance
type, institution or country, claim family, or date window.
