...
- It is one JSON object with the keys `id`, `title`, `type`, `xField`, `series` and `data`.
...
The chart `type`, the `id` text, axis labels, and whether an `annotations` list is present do not matter.
